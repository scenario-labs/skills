...
When a supported operation becomes available, publish the already-staged IDs through that operation. Verify the asset record reports public visibility, then check retrieval and download using a user-authorized scope outside the owning team. A scope merely appearing in `teams_list` is not authorization to use it for this check. Failed retrieval or download leaves verification incomplete; viewing an app link is not a substitute. For model-input templates, test reference use within the agreed generation budget before claiming that workflow validated; a dry run alone is not that proof.
