...
| 429 with `actionName`/`actionLimit` in `details`                       | Per-team concurrency ceiling (`parallel-custom-jobs` for generation): launch `wait=false`, hold `actionLimit` jobs in flight, let `jobs_wait` retire them; an immediate retry repeats it                                  |
...
| 400 `At least one of query, filter, image, or images must be provided` | `search` never lists bare: a "newest first" listing is `filter="createdAt EXISTS"` with `sort_by=["createdAt:desc"]`                                                                                                      |
