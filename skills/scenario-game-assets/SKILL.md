...
5. Transparency: `recommend` for a native-alpha generator first, since it makes the icons transparent at the source and skips a billed removal pass per icon. Fall back to a removal model only when the chosen generator lacks alpha: `recommend` for background removal (Photoroom, Pixelcut, 851 Labs at authoring time), `model_schema_get` the pick, then `model_run` with its image field set to the generated asset id.
