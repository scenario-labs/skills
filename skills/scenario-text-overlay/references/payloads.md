...
| Field           | Type             | Rules                                                               |
| --------------- | ---------------- | ------------------------------------------------------------------- |
| `variables`     | `[{key, value}]` | string values only (stringify numbers); duplicate keys are an error |
...
After uploading the PNG with `upload_asset`, store the same payload JSON
as the asset's description with `asset_update`, and tag it with
`asset_add_tags` (additive; `asset_update`'s `tags` field replaces the
whole set). The template then lives on the platform next to its render,
and any variant can be regenerated by editing `variables` alone.

## Example payloads

Text layer, a legal super pinned to the lower third:
...
Rich layer, a gradient headline card:
