...
9. Assemble on-platform: discover the compositor with `search` `query="compose video"` (a tool-model lane per `scenario-video-assembly`) and layer the score strip and channel bug from `scenario-text-overlay` in the safe corners; a local editor is the wrong lane. A succeeded compose job is not proof the layers drew: pull a frame from the composite (the free `firstFrame`) and confirm every layer is present and placed before delivering. Then cut 9:16 per `scenario-formats`' reframe lane (`search` `query="reframe"`; the master's `editCapabilities` advertise it), keeping the subject in frame (a blind cover-crop loses them). `asset_display` both.
