...
Script in six spoken beats: hook (one concrete tension or result), context (why this speaker cares), product moment, proof (visible demo or a user-supplied fact), turn (objection answered or before-after), close (soft CTA). Write for the mouth, not the page: contractions, false starts allowed, no taglines. Spoken pace runs near 2.5 words a second, so a 25-second ad is roughly 60 words; avatar clip runtime is not word count, though: members pad sentences with silence, so cut the silences out into separate clips and assemble those, never as stacked layers of the one clip, which mixes its audio with itself.
...
9. Assemble per `scenario-video-assembly`: talking head as the spine, insert cut over beats three and four, captions burned into the platform's safe zone, product-name card from `scenario-text-overlay` as an image layer. A succeeded compose job is not proof the layers drew or landed where sent: pull a frame from the composite and confirm the captions and the card are present and placed before delivering. Audit the audio too after any trim-and-reassemble, since a frame cannot show it: the master's speech duration with silence excluded (`silencedetect` gives both numbers) must not exceed the source's, and the sentence gaps must survive; more speech than the source contained means segments overlap, and it ships sounding doubled.
