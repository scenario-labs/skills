...
5. `search` `target="models"`, `query="avatar"`, `public=true`; pick by input contract (a script-taking member when the speech exists only as text, an audio-taking one when a recording exists), then by whether `model_schema_get` exposes a caption or subtitle switch: some members burn in gibberish subtitles that no negative prompt suppresses, and the ones that can be told not to cost several times more, so quote both.
