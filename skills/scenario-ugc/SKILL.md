...
| Recorded delivery, different face   | Motion-control members (`video2video`): character still plus the recording as driving `video`, `keepOriginalSound` keeps the voice | `scenario-kling`                                      |
| Generated creator speaking natively | Native-audio video families (`txt2video`), dialogue in quotes with the delivery named (pace, tone, one gesture)                    | `scenario-veo`, `scenario-seedance`, `scenario-kling` |
| Faceless voiceover                  | B-roll clips (`txt2video`) plus TTS narration (`txt2audio`)                                                                        | `scenario-video`, `scenario-elevenlabs`               |
