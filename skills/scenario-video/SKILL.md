...
Plan gating is common here: `recommend` marks such members `requires_plan_upgrade`, and running one anyway returns 403 `ModelAccessRestrictedError` naming `modelId` and `requiredPlan`, which no retry clears; pick an available member or surface the upgrade.
