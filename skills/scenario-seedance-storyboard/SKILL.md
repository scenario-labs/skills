...
Draw the panels in the register you want delivered: photoreal panels return filmic footage, while pencil panels leave the model to invent every surface and drift. Keep panel numbers small and low contrast, or under the frame: a large in-frame badge can burn into the render for a whole clip, a negative clause does not reliably suppress it, and the delivered corners are worth checking. A photoreal board carries identity by itself here, which puts the plates' real work upstream, holding the cast steady while the board is drawn. Give each character its own plate rather than a combined sheet: 2.5 takes up to 30 `referenceImages` and the prompt binds them by index, so `@image1` is fighter A and `@image2` is fighter B, each at full canvas and unambiguous. Pick the plate and board model for legible in-image text, strict grid adherence, and `referenceImages` input so panels lock to the plates; shortlist with `search` (at authoring time `model_openai-gpt-image-2` did all three), since `recommend` can answer this step with style LoRAs.

Stop for approval before the first video run: show the script, then the plates, then the numbered board, and invite a reshoot by panel number. Re-running one panel is a single board call with that clause changed and the approved board as a reference, far cheaper than a video run on a board nobody checked. Unattended, record the assumption and continue.
