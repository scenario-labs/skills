...
The Blender steps run locally in a shell with Blender 4.x or later; everything that generates goes through MCP tools. When Blender is not installed, ask the user to install it: no MCP tool renders a mesh from chosen cameras.
...
2. `recommend` an image-to-3D model with the user's words ("untextured mesh from one image"), `model_schema_get`, `model_run` with `dry_run=true` to price (image-to-3D jobs are expensive), then `wait=false` and `jobs_wait`. `asset_download` the GLB and save it with `curl -L`.
3. `recommend` a text-to-skybox model and generate one panorama in the picture's style, prompting for "viewpoint standing in the middle of an open empty floor at eye level, the floor clearly visible all around, no people". Download it.
4. `blender ... render_grounded.py -- --glb hero.glb --out search --search`, then `python3 match_view.py hero.png search/cameras.json`. Take the top azimuth and elevation as `--az0` and `--elev`; open the matching `search/search_az..._el...png` next to the picture to confirm, since front and back can score alike.
5. `blender ... render_grounded.py -- --glb hero.glb --pano world.png --out views --az0 30 --elev 10 --only 00,02,04,06,08,10,12,14,top` (the eye ring has 16 cameras, 22.5 degrees apart; `--height` sets the panorama eye height, `--yaw` turns the world, `--zoom` the framing). Then `python3 compose_layout.py views`. Look at every layout: the clay must stand on the floor, with the shadow under it.
6. Upload each `layout_<key>.jpg`. `recommend` an image model for reference-guided repainting, and check that its `model_schema_get` lists `referenceImages` (1024 square, a high quality tier and an opaque background worked well). Price one with `dry_run=true`, then launch one `model_run` per view with `wait=false`, `referenceImages: ["<layout asset>", "<source asset>"]`, and:
...
8. Transparent set: `recommend` a background-removal model, read its image field from `model_schema_get`, run it on each repainted asset id (not on a new studio repaint), download, then `python3 studio_shadow.py views/clay_<key>.png cut_<key>.png studio_<key>.png`. Both sets now show the same subject at the same size.
