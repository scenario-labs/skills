...
4. `train` with `action: "upload_images"`, `model_id`, `images: [<asset ids>]`. OAuth connections also pass `team_id` and `project_id` on every `train` call (see the `scenario` skill).
