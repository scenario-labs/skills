...
For a dataset already attached to a model: `model_get` (via `scenario_tool_execute_read`) returns the full record including training images; `asset_get` on each id reads the stored caption (the asset `description`). To inspect image content at scale, `asset_analyze` (catalog, cost-bearing) answers questions such as "is a second character visible" or "does this match the style of the others". Fixes: rewrite a caption with `asset_update` (`metadata: {"description": "..."}`), remove an image with `train` `action: "delete_image"`, replace one with `action: "update_image"`. For a dataset still on disk, check dimensions and duplicates locally before spending uploads.
