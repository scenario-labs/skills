...
| `materials.<name>`         | `objects` (slot count), `examples` (up to 3 object names), `bounds.lo` and `bounds.hi` (world space; shot targets in film-config.md start from these), `base_color` (Principled Base Color or the viewport color, RGBA, `null` when absent), `has_image_textures` |
