...
**No single source dominates.** If one artist, film, or property supplies most of the board, the output reads as a knockoff of it. Never search a brand you intend to resemble.
...
- Load it: `search` with `filters.collection_ids: ["col_..."]`. Every hit carries its own `description`, so the annotations come back with the images.
