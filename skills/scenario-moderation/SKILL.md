...
Video changes the economics of a block. Several providers moderate the finished render, so a rejected clip has already been rendered in full, and a retry of the identical payload renders it again. Read `cuCost` on the failed row before anything else: a policy block is typically refunded, and failed jobs are reimbursed except xAI generations stopped by moderation (see `scenario`), but the reservation is not always released at once, so report the figure, confirm later with `usage` bounded by `start_date` and `end_date` to the job's day with `include=["usages"]` for the per-type totals (the headline is project-lifetime, and a team-gate screen shows as its own IP Detection line), and file the `job_id` with support if a charge stands. Then switch provider as above, prompt unchanged.
