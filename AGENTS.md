...
- The husky `pre-commit` hook runs the same checks as CI: `scripts/check-style.sh` (house style), `pnpm format:check` (prettier), `scripts/check-skill-files.sh` (supporting files next to a SKILL.md are linked and runnable), `scripts/check-spelling.sh` (cspell), and `scripts/validate-skills.sh` (spec validation).
