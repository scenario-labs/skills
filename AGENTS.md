...
- The husky `pre-commit` hook runs `pnpm validate`, the same checks CI runs as separate steps: `pnpm style` (house style), `pnpm format:check` (prettier), `pnpm skill-files` (supporting files next to a SKILL.md are linked and runnable), `pnpm spell` (cspell), and `pnpm spec` (spec validation). Each maps to a script in `scripts/`, defined once in `package.json`.
