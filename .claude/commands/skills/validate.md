...
- **No constant generative model ids.** They came from a `recommend` step (a capability) or a `search` step (a member known by name, or private). A named first-party singleton tool model is not a violation; a named generative model is.
